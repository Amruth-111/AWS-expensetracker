...
exports.forgotpass=async(req,res)=>{
    try{
        ...
        const data= await transEmailApi.sendTransacEmail({
            ...
            htmlContent:`<a href="http://13.49.241.184:8081/password/reset_password/${id}">Reset password</a>`
...
exports.resetpass=async(req,res)=>{
    try{
        ...
        const forget=await forgotpassword.findOne({where:{id:id}})
        console.log(forget)
        if(forget){
            await forget.update({ active: false});
    res.send(`<html>
            <script>
